...
- 新增模态框和确认对话框组件
- 新增CreateOpportunityModal组件用于从客户详情页创建商机
- 新增状态管理和数据持久化
- 新增Stage枚举、Opportunity接口、FunnelState接口等类型定义
- 新增API服务层支持完整的商机管理功能
...
## 简介
本规范文档面向销售AI CRM系统的前端组件库，基于现有代码实现，定义了Sidebar导航、Header头部、StatsCards统计卡片、SalesFunnel销售漏斗、AIAudioAnalysis音频分析、DailySchedule日程管理、MapMiniView地图视图、AIBanner智能横幅、CreateOpportunityModal创建商机模态框等核心组件的设计原则、视觉样式、交互行为、尺寸规格与状态变化。文档同时提供组件组合模式与布局规范，帮助开发者在保持一致性的前提下扩展与维护UI。

**更新** 销售漏斗组件已从简单的静态展示升级为包含拖拽、编辑、删除、添加等完整交互功能的生产级看板系统，新增了完整的销售机会管理功能。

## 项目结构
组件库采用按功能模块划分的组织方式，核心组件位于 src/components 目录，页面组件位于 src/pages 目录，应用入口在 src/App.tsx 中进行整体布局编排。状态管理通过 Zustand store 实现，数据类型定义在 src/types 中统一管理，API服务层位于 src/services 目录。
...
MAP["MapMiniView.tsx"]
CREATE_OPPORTUNITY["Customers/CreateOpportunityModal.tsx"]
end
subgraph "状态管理"
...
- [CreateOpportunityModal.tsx:1-316](file://crm-frontend/src/components/Customers/CreateOpportunityModal.tsx#L1-316)
...
- [api.ts:158-178](file://crm-frontend/src/services/api.ts#L158-178)
...
**更新** 销售漏斗组件现为完整的生产级看板系统，包含拖拽、编辑、删除、添加等交互功能，新增了CreateOpportunityModal组件用于从客户详情页创建商机。
...
- **新增** CreateOpportunityModal 创建商机模态框：用于从客户详情页创建新的销售机会，包含完整的表单验证和API调用。
...
## 架构总览
组件库遵循"布局容器 + 功能组件"的分层结构。App.tsx 负责整体布局（侧边栏 + 主内容区），主内容区内部通过网格与列布局组织多个功能组件，确保信息密度与可读性的平衡。新增的API服务层提供统一的数据访问接口，支持完整的CRUD操作。

**更新** 销售漏斗组件现在使用Zustand状态管理，支持数据持久化和实时更新，新增了完整的API服务层支持。
...
API["services/api.ts API服务层"]
CREATE_OPPORTUNITY["CreateOpportunityModal 模态框"]
APP --> LAYOUT
LAYOUT --> SIDEBAR
LAYOUT --> HEADER
LAYOUT --> MAIN
...
### SalesFunnel 销售漏斗
**更新** 销售漏斗组件已升级为完整的生产级看板系统，包含丰富的交互功能和完整的销售机会管理能力。
...
- 数据结构
  - **新增** Stage 枚举：new_lead、contacted、solution、quoted、negotiation、procurement_process、contract_stage、won
  - **新增** Opportunity 接口：id、customerId、customerName、title、stage、value、probability、owner、priority、expectedCloseDate、lastActivity、description、nextStep
  - **新增** FunnelState 接口：opportunities 数组、selectedStage 状态、CRUD 操作方法
  - **新增** FunnelStage 接口：name、count、percentage、value、color
- 交互行为
  ...
  - **新增** **API集成**：通过opportunityApi进行完整的CRUD操作，支持数据持久化。
- 尺寸规格
...
- 状态变化
  ...
  - **新增** 状态管理使用Zustand，支持数据持久化和实时更新。
- 使用建议
  - 确保每个阶段都有合理的成交概率预设值。
  - 为高优先级机会提供更醒目的视觉标识。
  - 定期清理已完成或失败的销售机会。
  - **新增** 利用API服务层进行数据操作，确保数据一致性。
...
**新增** 用于从客户详情页创建新的销售机会的模态框组件。

- 视觉样式
  - 固定居中显示，背景半透明遮罩层。
  - 头部包含标题、客户信息和关闭按钮。
  - 表单区域包含项目名称、预计金额、销售阶段、成交概率、优先级、预计成交日期、下一步行动、项目描述等字段。
  - 底部包含取消和创建按钮。
- 数据结构
  - **新增** OpportunityFormData 接口：title、value、stage、probability、priority、expectedCloseDate、description、nextStep
  - **新增** STAGE_PROBABILITIES 映射：各阶段的默认成交概率
- 交互行为
  - 输入验证：项目名称必填，金额为数字格式。
  - 阶段变更：自动更新对应的默认成交概率。
  - 加载状态：提交时显示加载动画。
  - 错误处理：显示API调用错误信息。
- 尺寸规格
  - 模态框宽度最大100%，最大高度90vh，支持滚动。
  - 表单字段采用统一的内边距和字体大小。
- 状态变化
  - 表单提交时显示加载状态，成功后调用onSuccess回调并关闭模态框。
  - 错误状态下显示红色提示框。
- 使用建议
  - 在客户详情页中调用，传入customerId和customerName参数。
  - 处理onSuccess回调以更新父组件状态。
  - 设置合适的默认值和验证规则。
...
- 组件间依赖
  ...
  - **新增** CreateOpportunityModal 依赖 API 服务层进行数据操作。
- 外部依赖
...
TYPES["types/index.ts"]
API["services/api.ts"]
CREATE_OPPORTUNITY["CreateOpportunityModal.tsx"]
PKG --> LUCIDE
PKG --> TW
PKG --> INTER
PKG --> ZUSTAND
...
- 可访问性
  ...
  - **新增** 表单字段提供aria-label和aria-describedby属性。
  - **新增** 错误提示使用语义化的颜色和图标。
...
- **新增** API调用失败
  ...
  - 确认请求参数格式和必填字段。
- **新增** 数据持久化问题
  - 检查浏览器存储空间和localStorage权限。
  - 验证persist中间件配置和序列化/反序列化过程。
...
## 结论
本组件库规范以现有代码实现为基础，明确了各组件的视觉、交互与布局规范，并提供了组合与使用建议。销售漏斗组件的升级代表了从静态展示到生产级应用的重要转变，新增的拖拽、编辑、删除、添加等功能使其成为完整的销售管理工具。新增的CreateOpportunityModal组件进一步完善了销售机会管理功能，支持从客户详情页直接创建商机。建议在后续迭代中补充类型约束、测试用例与无障碍属性，以进一步提升质量与可维护性。
...
- 最佳实践
  ...
  - **新增** API调用使用错误处理和重试机制，提升系统稳定性。
  - **新增** 表单验证在客户端和服务器端双重保障，确保数据质量。
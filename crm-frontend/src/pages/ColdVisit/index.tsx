...
export default function ColdVisit() {
  ...
  // 历史记录状态
  const [history, setHistory] = useState<ColdVisitRecord[]>([]);
  ...
  const fetchHistory = async () => {
    try {
      ...
      const response = await coldVisitApi.getHistory({ limit: 20 });
  ...
  const handleConvertToCustomer = async () => {
    ...
    try {
      const convertResult = await coldVisitApi.convert(result.id, {
      ...
      // 跳转到客户详情页
      navigate(`/customers/${convertResult.data.customer.id}`);
    } catch (err) {
  ...
  // 从历史记录加载
  const handleLoadFromHistory = (record: ColdVisitRecord) => {
    setResult(record);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">陌生拜访AI助手</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">智能分析企业信息，生成拜访话术，开拓新客户</p>
        </div>
          ...
          {!showHistory ? (
            <>
              ...
              <div className="bg-white dark:bg-slate-900 rounded-xl p-6 border border-slate-200 dark:border-slate-800">
                <h3 className="font-semibold text-slate-900 dark:text-white mb-4">分析企业</h3>
                ...
                {/* 错误提示 */}
                {error && (
                  <div className="mt-4 px-4 py-3 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-lg text-sm">
                    {error}
                  </div>
                )}
                ...
                <button
                  ...
                  className="mt-4 w-full py-3 bg-primary text-white rounded-xl font-semibold shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {isAnalyzing ? (
                    <>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
              <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
                <h3 className="font-semibold text-slate-900 dark:text-white">历史分析记录</h3>
                <button
                  ...
                  <div className="space-y-3">
                    {history.map((record) => (
                      <button
                        ...
                        className="w-full p-4 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl text-left transition-colors"
                      >
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                            ...
                            {record.status === 'converted' ? '已转换' : '待跟进'}
                          </span>
                        </div>
        ...
        <div className="lg:col-span-2">
          {result ? (
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
              {/* 企业基本信息卡片 */}
              <div className="bg-gradient-to-br from-primary/5 to-primary/10 dark:from-primary/10 dark:to-primary/20 p-6">
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800 rounded-xl p-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {intelligence.businessScope.map((item: string, index: number) => (
                          <span
                            key={index}
                            className="px-3 py-1.5 bg-white dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-lg text-sm border border-slate-200 dark:border-slate-600"
                          >
                            {item}
                          </span>
                        ))}
                        ...
                        <div className="space-y-4">
                          {intelligence.recentNews.map((news, index) => (
                            <div key={index} className="flex gap-3">
                              <span className="text-xs text-slate-400 dark:text-slate-500 shrink-0 pt-0.5 w-20">
                                {news.date}
                              </span>
                              <div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl p-12 border border-slate-200 dark:border-slate-800 text-center">
              <div className="size-20 mx-auto rounded-2xl bg-primary/10 flex items-center justify-center mb-4">
              ...
              <p className="text-slate-500 dark:text-slate-400 max-w-md mx-auto">
                输入公司名称或上传企业照片，AI将为您分析企业信息、生成个性化拜访话术
              </p>
            </div>
          )}
        </div>
...
function ContactCard({ contact }: { contact: KeyContact }) {
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-800 rounded-xl p-4 flex items-center gap-4">
      <div className="size-12 rounded-xl bg-white dark:bg-slate-700 flex items-center justify-center shrink-0 border border-slate-200 dark:border-slate-600">
